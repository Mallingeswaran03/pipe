...
const stripe = require('stripe')('sk_test_51P0dthSIH9yAlEwXPpzXV0DwTVboPZnZsBn5XImscRzo4LlWqWKRLt1G5RCXxyWhvQYfitSwyyH8wT8y3ZsCpWby00Vx5zW2PS')
...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'ambalupvc@gmail.com',
    pass: 'gsyl dfzd tihd erxz'
  },
});
...
app.post('/AddProduct', upload.single('addpimg'), (req, res) => {
  ...
  const datep = getdate();
  db.query(
    'INSERT INTO product (cid,scid,name,price,quantity,description,image,datep,overview,brand,discount,price1) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
    ...
    req.body.addoverview, req.body.addbrand, req.body.discount, req.body.addtpprice],
    (err, result) => {
      if (err) {
        console.log(err);
        res.send("failed")
...
app.post('/AddProductup', upload.single('addpimg'), (req, res) => {
  ...
  const datep = getdate();
  db.query(
    'update product set name=?,price=?,quantity=?,description=?,image=?,overview=?,brand=?,discount=?,price1=? where pid=?',
    [req.body.addpname, req.body.addpprice, req.body.addpqty, req.body.addpdescription, addpimg, req.body.addoverview, req.body.addbrand, req.body.discount, req.body.addtpprice, req.body.pid],
    (err, result) => {
      if (err) {
        console.log(err);
        res.send("failed")
...
// ------------------------------fnckout--------------------------
app.post("/fnckout1212", async (req, res) => {
  try {
...
app.post("/fnckout", async (req, res) => {
  try {
    ...
    transporter.sendMail({
      from: '"Ambaal Pipes" <ambalupvc@gmail.com>',
      to: req.body.email,
      subject: "Your Order hasbeen placed successfully",
      text: `Hi,${req.body.name} Order hasbeen placed successfully,
 Bill Number : ${billno}
 Placed On : ${datep}
 Address : ${req.body.address}
 You may track your status ay Myaccount->My Orders`
...
// ------------------------------fnckout--------------------------
app.post("/fnckout1", async (req, res) => {
