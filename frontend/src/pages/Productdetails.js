              ...
              return (
                <>
                  ...
                  <div className='col-xl-5 col-md-6 col-sm-6' style={{ padding: '20px', margin: "auto" }}>
                    ...
                    {result.discount===0?<>
                      <h4 style={{ color: 'green'}}>Price : Rs.{result.price}.00</h4>

                       </>
                       :
                       <>
                       <br/>
                       Discount : {result.discount} %<br/>
                       <h5 style={{ color: 'gray',textDecoration:"line-through" }}>{result.price1}.00</h5>
                       <h4 style={{ color: 'green'}}>Price : ₹ {result.price}.00</h4>
                       <br/>
                       </>
                    }
...
<div className='container-md'>
  {isLoggedIn &&
      <form id='addr' onSubmit={addr} className='col-md-8 offset-md-2' style={{ border: '1px solid rgb(218, 208, 208)', borderRadius: "10px", padding: "20px", backgroundColor: "#fff", boxShadow: "0px 4px 10px rgba(0, 0, 0, 0.1)" }}>
        <div className='row mb-3'>
        <div className='col-md-12e:\projects\react\ifbpoint\frontend\src\pages\Cart.js'>
        <h4 className='mb-4'>Review</h4>
      </div>
          <div className='col-md-6'>
            <label className='form-label'>User Name *</label>
            <input className='form-control' type="text" readOnly value={localStorage.getItem("userName")} />
          </div>
          <div className='col-md-6'>
            <label className='form-label'>Email *</label>
            <input className='form-control' type="email" readOnly value={localStorage.getItem("userEmail")} />
          </div>
        </div>
        <div className='row mb-3'>
          <div className='col-md-6'>
            <label className='form-label'>Rating *</label>
            <select className='form-select' id="rtst" onChange={e => setRtst(e.target.value)} style={{ borderRadius: "5px", boxShadow: "0px 2px 5px rgba(0, 0, 0, 0.1)" }}>
        ...
        <div className='row mb-3'>
          <div className='col'>
            ...
            <textarea id="co" className='form-control' rows="4" required onChange={e => setCo(e.target.value)} style={{ borderRadius: "5px", boxShadow: "0px 2px 5px rgba(0, 0, 0, 0.1)" }}></textarea>
          </div>
        </div>
        <div className='row justify-content-center'>
          <div className='col-md-4'>
            <button type="submit" className="btn btn-danger btn-sm w-100" style={{ borderRadius: "5px", boxShadow: "0px 2px 5px rgba(0, 0, 0, 0.1)" }}>Send</button>
          </div>
        </div>
      </form>
